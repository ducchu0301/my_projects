...
function getPagingQuery($sql, $itemPerPage = 10, $pageName)
{
	if (isset($_GET[$pageName]) && (int)$_GET[$pageName] > 0) {
		$page = (int)$_GET[$pageName];
	} else {
		$page = 1;
	}
...
*/
function getPagingLink($sql, $itemPerPage = 10, $strGet = '', $pageName)
{
	$result        = dbQuery($sql);
	$pagingLink    = '';
	...
	if ($totalPages > 1) {
		...
		if ($pageNumber > 1) {
			$page = $pageNumber - 1;
			if ($page > 1) {
				$prev = " <a href=\"$self?$pageName=$page&$strGet/\">[Prev]</a> ";
			} else {
		...
		if ($pageNumber < $totalPages) {
			$page = $pageNumber + 1;
			$next = " <a href=\"$self?$pageName=$page&$strGet\">[Next]</a> ";
			$last = " <a href=\"$self?$pageName=$totalPages&$strGet\">[Last]</a> ";
		} else {
		...
		for($page = $start; $page <= $end; $page++)	{
			if ($page == $pageNumber) {
				...
				if ($page == 1) {
					...
					$pagingLink[] = " <a href=\"$self?$pageName=$page&$strGet\">$page</a> ";
				}	
			}
...
function deleteGET($nameGET) {
	$shoppingReturnUrl = isset($_SESSION['shop_return_url']) ? $_SESSION['shop_return_url'] : 'index.php';
	$array=explode("?",$shoppingReturnUrl);
	if(count($array)==2){
		$shoppingReturnUrl=$array[1];
		$array2=explode("&",$shoppingReturnUrl);
		$shoppingReturnUrl='';
		for($i=0;$i<count($array2);$i++) {
			if($i==count($array2)-1) {
				$shoppingReturnUrl=$shoppingReturnUrl.$array2[$i];
			}
			else $shoppingReturnUrl=$shoppingReturnUrl.$array2[$i]."=";
		}
		$array3=explode("=",$shoppingReturnUrl);
		$shoppingReturnUrl='';
		$flag=-1;
		for($i=0;$i<count($array3);$i=$i+2) {
			if($array3[$i]==$nameGET) $flag=$i/2;
		}
		for($i=0;$i<count($array2);$i++) {
			if($i==count($array2)-1&&$i!=$flag) $shoppingReturnUrl=$shoppingReturnUrl.$array2[$i];
			else if($i!=$flag) $shoppingReturnUrl=$shoppingReturnUrl.$array2[$i]."&";
		}
	}
	else $shoppingReturnUrl='';
	return $shoppingReturnUrl;
}
?>